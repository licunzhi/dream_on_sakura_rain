...
import com.meimei.quartz_demo.job.CountJob;
...
public class JobController {
    ...
    public void addJob(@PathVariable(value = "jobName") String jobName) {
        ...
        try {
            ...
            JobDetail jobDetail = JobBuilder.newJob(SampleJob.class).withIdentity(jobName, groupName).build();
            //执行的任务中传入参数
    ...
    public void addJobOne(@PathVariable(value = "jobName") String jobName) {
        ...
        try {
            ...
            //构建一个新的任务规范，执行特定任务，任务执行的时间
            JobDetail jobDetail = JobBuilder.newJob(JobOne.class).withIdentity(jobName, groupName).build();
            //执行的任务中传入参数
    ...
    //创建一个新的job任务 执行执行的次数失效
    @GetMapping("/add/count/{jobName}")
    public void countJob(@PathVariable(value = "jobName") String jobName) {
        String groupName = "group_one"; //定义job所在组名称
        String cronExpression = "/10 * * ? * * *";//执行时间表达式
        try {

            //构建一个新的任务规范，执行特定任务，任务执行的时间
            JobDetail jobDetail = JobBuilder.newJob(CountJob.class).withIdentity(jobName, groupName).build();
            //执行的任务中传入参数
